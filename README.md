...
```
oc create -f image-streams/s2i-minimal-notebook.json
```
...
```
oc create -f build-configs/s2i-minimal-notebook.json
```
...
oc new-build --name custom-notebook \
  ...
  --code https://github.com/ruo91/jupyter-notebooks \
...
```
oc create -f build-configs/s2i-scipy-notebook.json
oc create -f build-configs/s2i-tensorflow-notebook.json
```
...
```
oc create -f templates/jupyter-notebook.yaml
oc create -f templates/jupyter-notebook-pv.yaml
```
